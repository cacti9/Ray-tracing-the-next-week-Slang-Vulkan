...
namespace vma_raii { 
  ...
  void Allocator::destroyBuffer(vk::Buffer& buffer) {
    if (!buffer) {
      return;
    }
    auto bufferIt = std::ranges::find_if(buffers, [buffer](auto const& allocation) {
  ...
  std::pair<vk::Buffer, void*> Allocator::createMappedBuffer(VkDeviceSize size) {
    return createMappedBuffer(size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
  }
  std::pair<vk::Buffer, void*> Allocator::createMappedBuffer(VkDeviceSize size, VkBufferUsageFlags usage) {
    buffers.push_back(std::make_unique<MappedBuffer>(size, usage));
  ...
  std::pair<vk::Buffer, void*> Allocator::createReadbackBuffer(VkDeviceSize size) {
    buffers.push_back(std::make_unique<ReadbackBuffer>(size));
    auto& back = buffers.back();
    void* mapped;
    if (vmaMapMemory(vmaAllocator, back->allocation, &mapped) != VK_SUCCESS) {
      throw std::runtime_error("vma failed to map readback buffer");
    }
    return { back->buffer, mapped };
  }
  void Allocator::invalidateBuffer(vk::Buffer buffer, VkDeviceSize size) {
    auto bufferIt = std::ranges::find_if(buffers, [buffer](auto const& allocation) {
      return allocation->buffer == buffer;
    });
    if (bufferIt == buffers.end()) {
      throw std::runtime_error("vma failed to find buffer for invalidation");
    }
    if (vmaInvalidateAllocation(vmaAllocator, (*bufferIt)->allocation, 0, size) != VK_SUCCESS) {
      throw std::runtime_error("vma failed to invalidate buffer");
    }
  }
  void Allocator::destroyImage(vk::Image& image) {
    if (!image) {
      return;
    }
    imagesMap.erase(image);
