...
namespace vma_raii {
  ...
  private:
    ...
    struct MappedBuffer : Buffer {
      MappedBuffer(VkDeviceSize size, VkBufferUsageFlags usage)
        : Buffer(size, usage, VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT | VMA_ALLOCATION_CREATE_MAPPED_BIT) {}
      ~MappedBuffer() override {
        vmaUnmapMemory(vmaAllocator, allocation);
      }
    };

    struct ReadbackBuffer : Buffer {
      ReadbackBuffer(VkDeviceSize size)
        : Buffer(size, VK_BUFFER_USAGE_TRANSFER_DST_BIT, VMA_ALLOCATION_CREATE_HOST_ACCESS_RANDOM_BIT | VMA_ALLOCATION_CREATE_MAPPED_BIT) {}
  ...
  public:
    ...
    std::pair<vk::Buffer, void*> createReadbackBuffer(VkDeviceSize size);
    void invalidateBuffer(vk::Buffer buffer, VkDeviceSize size);
